...
// #endif
    ...
    double Pe_h=0.5*mod2_vel*h_eff/(_IRe*mu_tp);
...
//       _mu_turb=eval_var1(val_tbg);
    ...
    double IRe_eff  =_IRe*mu_tp/* +_IRe*les*h_eff*h_eff*sqrt(2*_sP) */ ;   //  visc_eff  at g pt
...
//
        for(int j=0; j<el_ndof2; j++) {
          ...
          KeM(indx,j+ivar*el_ndof2) +=dtxJxW_g*rho*(
                                        (Adv_g+ unsteady_flag*(1./_dt+1.2)*phij_g)* (phii_g+Phi_supg) // time + advection
