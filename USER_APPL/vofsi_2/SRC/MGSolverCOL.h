#ifndef __mgsolverdddcol_h__
#define __mgsolverdddcol_h__
...
public:
  ...
  /// Level constructor
  MGSolCOL(MGEquationsSystem& mg_equations_map_in,
	  const int vars_in[],
            std::string eqname_in="CO_F",
            std::string varname_in="phase"
           );
...
//  void bc_intern_read(double x[],int bc_Neu[],int u[]); ///< Read ic.
  ...
  /// Color Function.
  void ColorFunc(const double time,const int Level,const int mode);
//   /// Surface Assemblying
//   void GenMatRhsB(const double time, const int Level,const int mode);
  ...
  void MGTimeStep_nl_setup(const double time, const int /*iter*/) ;
