...
//
//
// //   int idx_fsi= _data_eq[2].tab_eqs[FS_F];// Navier-Stokes ---------------------------------
// //   _FSI_idx=(idx_fsi>=0)? _data_eq[2].indx_ub[idx_fsi]:-1;
// //
// //   int idx_ds= _data_eq[2].tab_eqs[SDSX_F];// Turbulence --------------------------------------
...
//   int interface;
//   double rho;
  double l_old[DIMENSION*NDOF_FEM]; double  p_old;
  // gauss integration  ------------------
  ...
  for(int iel=0; iel < (nel_e - nel_b); iel++) {
    ...
    for(int idim=0; idim<DIMENSION; idim++) {
      ...
      for(int d=0; d< NDOF_FEM; d++) {   // element nodes xxg (DIM)
        ...
        flag_group[d]=ext_mesh->_bc_id[el_conn[d]];
      }
    }
    ...
    phase=(ext_mesh->_mat_id[iel+nel_b]==2)?0:1; // phase from _mat_id

    //  fluid liquid phase ======================================================================
    for(int iside=0; iside< el_sides; iside++)  {//  fluid boundary  ----------------------------
      if(el_neigh[iside] == -1) {
...
//             flag_sur_group[idof]=ext_mesh->_bc_id[el_dof_indices[idofb]];
...
//     cout << "bc KeM" << endl; cout << KeM << endl; cout << "FeM" << endl; cout << FeM << endl;
...
//                      std::cout << "l KeM \n" << KeM << " ielem \n" <<  iel << " rhs \n" <<  FeM;
      ...
      // Volume -----------------------------------------------------------------------------------
      matrixrhsvol_sol_ds(KeM,FeM, el_ndof,flag_group);
//                       std::cout << "s KeM \n" << KeM << " ielem \n" <<  iel << " rhs \n" <<  FeM;
    }// ------------------------------------------------------------------------------------------
//     double max_diag=0.; int count=0;
//     for(int id=0; id< el_mat_nrows; id++) {  if(_bc_el[id] !=0 && flag_group[id] <999) if(max_diag<fabs(KeM(id,id))) { max_diag=fabs(KeM(id,id)); }  }
//     for(int id=0; id< el_mat_nrows; id++) if(_bc_el[id]==0 && flag_group[id] <999) {
//         const double pivot=fabs(KeM(id,id));
//         if(fabs(pivot)<1.e-10) {     std::cout << " rrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrr\n";           std::cout << "vol KeM \n" << KeM << " ielem \n" <<  iel << " rhs \n" <<  FeM;    }
...
//       if(fabs(disp) > maxf)   maxf=fabs(disp);
//           }
//           if(maxf<0.1)   maxf=0.1;
//
//     for(int inode=0; inode<n_nodes; inode++) {
//       double disp=(*x_old[_NoLevels-1])(inode);
// //       _mgmesh._xyz[inode+offsetp] = _mgmesh._xyzo[inode+offsetp]+disp;
//     _mgmesh._xyz[inode+offsetp] = _mgmesh._xyzo[inode+offsetp]+(1)*disp;
//       _mgmesh._dxdydz[inode+offsetp] =0.;
//     }
// // #endif
...
// // =========================================================================================
// /// This function controls the assembly and the solution of the T_equation system:
// void MGSolDS::MoveMesh(const int Level) { // ------------------------------------
//
//
//
//
...
// // //       _mgmesh._dxdydz[inode+offsetp]= (*disp[_NoLevels-1])(inode);
// // //     }
// //    _mgmesh.MoveMesh(_dir,*x_old[_NoLevels-1]);
//
// // #endif
//   }
//   // ==============================================================
...
//     interp_el_gdx(_data_eq[2].ub,_FF_idx[FS_F], _nFSIdim,_dphi_g[2],el_ndof2,vel_gdx);    // derivatives  vel_gdx[DIM][DIM]
//     interp_el_gddx(_data_eq[2].ub,_FF_idx[FS_F], _nFSIdim,_ddphi_g[2],el_ndof2,vel_gddx);
...
#ifdef AXISYM
      JxW_g2  *=xyz_g[0]; //JxW_g1 =JxW_g2;  // std::cout<<_ub_g[2][0]<<"   "<<std::endl;
#endif
      if(el_conn[qp]==94&& _dir==0){
      for(int idim=0; idim<  _nDSdim; idim++) {  // old  Velocity at gaussina point q
         vel_g[idim]=0.;  
         vel_g[idim] += _data_eq[2].ub[qp+(idim+_FF_idx[SDSX_F])*NDOF_FEM];
      } 
         value  += /*JxW_g2**/ ((vel_g[0]-0.07) *(vel_g[0]-0.07)     );   // functional
     }
...
// ----------------------------------------------------------
#ifdef PRINT_INFO
  std::cout << " functional(DS): computed  " <<  value   << std::endl;
#endif
