...
///   simm=0, disp_in0=1,disp_tg0=2,fix_disp=3, -------------------------------
///   disp_in=5,disp_tg=6,wall_disp=7, ----------------------------------------
///   Neumann (integratral constraint >9) -------------------------------------
///   free_disp=10, free_dispn=12,interior=11,slip=13, ------------------------
// ----------------------------------------------------------------------------

  bc_Neum[0]= fix_disp;bc_flag[0]=0;
  if      (bc_gam< 20)  {bc_Neum[0]= fix_disp;  }  //fixed displacement fluid boundary
  else if (bc_gam<30)   {bc_Neum[0]= free_disp; }  //free movinng surface solid
  else if (bc_gam>900)  {bc_Neum[0]= fix_disp;  }  //fixed interaface 
  if      (bc_gam== 22)  {bc_Neum[0]= fix_disp;  }  //fixed displacement fluid boundary
  return;
 }
#endif
// ============================================================================
