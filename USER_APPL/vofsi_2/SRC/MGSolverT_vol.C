...
//     double cc=_msolcc->get_2phase(block,xyz_g); // new cc
    ...
    for(int i=0; i<el_ndof2; i++)     {
      ...
      for(int idim=0; idim< _nTdim; idim++) {
        ...
        Phi_supg += _T_parameter.SUPG*_bc_el[i]* f_upwind*vel_g[idim]* dphiidxg; // phii_g+
      }
...
//                     - (1.-_euler_impl)*rhocp*Adv_expl*(phii_g+Phi_supg)   // explicit advection
//                     - (1.-_euler_impl)*Lap_expl                           // explicit diffusion
//                     + (1.-_euler_impl)*Lap_supg*Phi_supg                  // explicit diffusion supg
 cc*       (phii_g+Phi_supg)
                  );
      ...
      for(int j=0; j<el_ndof2; j++) {
        ...
        for(int idim=0; idim<  _nTdim; idim++) {
          ...
          Lap += _T_parameter.UPWIND*f_upwind*vel_g[idim]*vel_g[idim]*dphijdxg*dphiidxg;             // normal upwind 
          for(int jdim=idim+1; jdim< idim+ _nTdim; jdim++)  Lap += _T_parameter.UPWIND2*
            f_upwind*vel_g[(jdim%_nTdim)]*vel_g[idim]*_dphi_g[2][j+(jdim%_nTdim)*el_ndof2]*dphiidxg; // skew upwind Laplacian
        ...
        KeM(i,j) +=dtxJxW_g*( // energy-equation matrix
                     unsteady*phij_g*(phii_g +Phi_supg)// time term
//                      + _euler_impl*rhocp*Adv*(phii_g+Phi_supg)     //advection term
//                      + _euler_impl*Lap                             //diffusion term
//                      - _euler_impl*Lap_supgi*Phi_supg              //diff supg term
                   );
//         std::cout << "Matrix \n"  <<KeM  << "FeM\n"  <<FeM;
      }
    } // ----------------------------------------
