...
// // //         END VOF
    ...
    double Qad =_qheat;
    double rhocp_1=1e+0; double rhocp_2=1.e+1; double rhocp_3=1.e+1;

//     double rhocp_inter=((phase<0.5)?rhocp_1:rhocp_2);
    double rho=1.;
    double vof_phase= cc2/4;
    double rhocp= rhocp_3*vof_phase+(1-vof_phase)*((phase<0.5)?rhocp_1:rhocp_2);
//      double rhocp= ((phase<0.5)?rhocp_1:rhocp_2);
...
// //       source->eval(elem_med, 4, src_value);
// //       }
// //     }
// //     }
// #endif
...
#ifdef FSI_EQUATIONS
      const int ns_idx=_data_eq[2].indx_ub[_data_eq[2].tab_eqs[FS_F]];  // Navier-Stokes
...
//       std::cout << " alfa_eff" << alpha_eff << " ni " << _nut_ratio << " up 0 " <<  f_upwind[0]*vel_g[0]*vel_g[0] << " up 1 " <<  f_upwind[1]*vel_g[1]*vel_g[1] << " h 0 " <<  h_eff[0] << " h 1 " <<  h_eff[1]<< "\n";
      ...
      for(int i=0; i<el_ndof[2]; i++)     {
        ...
        double dtxJxW_g=JxW_g[2]*(_bc_vol[i]%2);
        ...
        if(mode == 1) {
          ...
          // rhs
          double src_phase=0;
          if((_ub_g[2][0]-0.5 )*(_ub_g[2][0]-0.5 )+
              (_ub_g[2][1]-0.5 )*(_ub_g[2][1]-0.5 ) < 0.1*0.1)
            src_phase=1;

          FeM(i) += dtxJxW_g*(
                      rhocp*_ub_g[2][ns_T]*phii_g/_dt   // time
//                       +Qad*phii_g                       // heat source
//                       +src_value[3]*phii_g                       // heat source
//                       +1.*phii_g
//                       +rhocp*phii_g*src_phase*10000*vof_phase
...
//             if(idim==DIMENSION) Adv += 1.*dphijdx_g[2][idim];
...
// #ifdef  HAVE_MED
// //  2D case
// // 	           if (fabs(bc_id[gl_node_bd])==11  ||  fabs(bc_id[gl_node_bd])==12 ) {
// // //  3D case
// //                  if (fabs(bc_id[gl_node_bd])==50  ) {
// // //                      int gl_node_med= ext_es->_interfaceFunMap[fabs(bc_id[gl_node_bd])]->NodeID()[gl_node_bd];
// // //                      ext_es->_interfaceFunMap[ fabs(bc_id[gl_node_bd])]->eval_node(gl_node_med,3, bc_value);
// // 		  }
// // 		   if ( fabs(bc_id[gl_node_bd])==111 ) {
// //                      int gl_node_med= ext_es->_interfaceFunMap[fabs(bc_id[gl_node_bd])]->NodeID()[gl_node_bd];
// //                      ext_es->_interfaceFunMap[ fabs(bc_id[gl_node_bd])]->eval_node(gl_node_med,3, bc_value);
// // 		  }
// #endif
              // bc for bc_Neum[0]=0; bc_flag[0]=1;
              FeM(lv_node) += bc_beta*Ipenalty*_data_eq[2].ub[ns_T*NDOF_FEM+lv_node];
...
// 	                printf(" \n  ystar %lf kappa %lf y %lf f_qn %lf",ystar/_IPrdl_turb,kappa_w,_y_dist,f_qn);
            ...
            for(int lsi_node=0; lsi_node< elb_ndof[2]; lsi_node++) {
              ...
              int bc_v=(int)(_bc_vol[lei_node]%2);
