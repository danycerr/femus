...
// ====================================================================================
// This function assemble the matrixis for the boundary conditions --------------------
// ====================================================================================
...
//             FeM(indx_row)  += -1.*bc_rhs*dtJxW_g*phii_g*(normal[ivar+_dir])*20;//(_ub_g[1][0]);
            ...
            double un=0;  for(int  kdim=0; kdim<_nDSdim; kdim++) { un +=u_old[sur_toply[i]+kdim*NDOF_FEM]*normal[kdim]; }
...
//             eff_stress =u_tau*u_tau/fabs(vel_bound[0]+1.e-20);
            ...
            for(int j=0; j<elb_ndof[2]; j++) {  // quad -quad --------------------------------------------
              ...
              for(int  jvar=ivar+_dir+1; jvar< _nDSdim+_dir; jvar++)    {  // u
                ...
                FeM(indx_row) +=   -1.*_data_eq[2].ub[_DS_idx*NDOF_FEM+sur_toply[j]+jvar1*el_ndof[2]]*
...
//           Ipenalty=1.;
          ...
          for(int   jvar=ivar+_dir+1; jvar< _nDSdim+ivar+_dir; jvar++)    {
            ...
            double valj=_data_eq[2].ub[(_DS_idx+jvar1)*NDOF_FEM+sur_toply[lbnode]];
...
//             int bc_yplus=((bc_s&8)>>3);  // (0;1??) nonhomogeneous
        ...
        if(ivar+_dir == dir_maxnormal)  {  // normal ---------------------------------------------------------------------------------------------------
          ...
          for(int   jvar=ivar+_dir+1; jvar< _nDSdim+_dir+ivar; jvar++)    {  // non-diagonal part  ->  u_n=n(u*nx+v*ny+w*nz)
            int      jvar1=jvar%DIMENSION;
            double aij= Ipenalty*normal[jvar1]*normal[ivar+_dir];
            double valj=u_old[jvar1*NDOF_FEM+sur_toply[lbnode]];
