// ===============================================================================
// --------------   FLUID-STRUCTURE-INTERACTION system [FSI_F] -------------------
// ===============================================================================
...
// class files --------------------------------------------------
#include "MGSclass_conf.h"     // Fluid-Structure-Interaction class conf file
#include "MGSolverFSI.h"       // Fluid-Structure-Interaction class header file
...
void MGSolFSI::set_bc_matrix(
  ...
  //   boundary condition (Diriclet normal=0; Neumann=1)
  //  ============================================================================
...
//   double InvJac[DIMENSION*DIMENSION];
  ...
  for(int  ivar=0; ivar< _nvars[2]; ivar++)    {
    ...
    if(bc_dn>0) {   // Neumann  -------------------------------------------------------------------------------------------------------------------
      ...
      if(bc_s !=3)for(int k=0; k< elb_ndof[_pres_order]; k++) { _bc_el[_pres_order*sur_toply[k]+ _nFSIdim*NDOF_FEM]= 0  ; }// pressure bc
//        if(bc_s > 3 && _pres_order == 0 ) _bc_el[_nFSIdim*NDOF_FEM]= 0;//piecewice pressure
     int flag_normal=(ivar+_dir == dir_maxnormal)?1:0 ;
      ...
      if(flag_normal)  {  // normal ---------------------------------------------------------------------------------------------------
        ...
        for(int  qp=0; qp< elb_ngauss; qp++) {  //gaussian integration loop (n_gauss)
          ...
          double  JxW_g=det*_fe[2]->_weight1[_nFSIdim-2][qp]; // weight
          _fe[2]->get_phi_gl_g(_nFSIdim-1,qp,_phi_g[2]);   // global coord _phi_g
          _fe[1]->get_phi_gl_g(_nFSIdim-1,qp,_phi_g[1]);   // global coord _phi_g
//                interp_el_sol(pressure,0,1,_phi_g[1],elb_ndof,_ub_g[1]);
          ...
          for(int i=0; i< elb_ndof[2]; i++) {  // Assemblying NS equation
            ...
            //  if(mode == 1)   {
            FeM(indx_row)  += -1.*bc_rhs*dtJxW_g*phii_g*(normal[ivar+_dir])*1.;//(_ub_g[1][0]);
            //    }
            // wall_turb boundary conditions
            double un=0;  for(int  kdim=0; kdim<_nFSIdim; kdim++) { un +=u_old[sur_toply[i]+kdim*NDOF_FEM]*normal[kdim]; }
            double ut=0;  for(int  jvar=0; jvar<_nFSIdim; jvar++) {
...
#endif
//                printf("\t\t\t\t\t\t\t log region vel_bound %f u_tau %f eff_stress %f \n", vel_bound[0], u_tau,eff_stress );
            ...
            for(int j=0; j<elb_ndof[2]; j++) {  // quad -quad ------------------------------------------------------------------------------------------
              ...
              KeM(indx_row,indx_row) +=   dtJxW_g*bc_tg*bc_normal*eff_stress*phij_g*phii_g
                                          ...
                                          //  -bc_tg )    // (?1?) -> tg
                                          ;
              for(int  jvar=ivar+_dir+1; jvar< _nFSIdim+_dir; jvar++)    {  // u
                int jvar1=jvar%DIMENSION;

                KeM(indx_row,sur_toply[j]+jvar1*el_ndof[2]) +=    dtJxW_g*eff_stress*phij_g*phii_g*bc_normal*bc_tg;
              }// jvar-loop
            }//  j-loop ---------------------------------------------------------------------------------------------------------
        ...
        for(int i=0; i< elb_ndof[_pres_order]; i++) {  // Assemblying NS equation
          const int   indx=_pres_order*sur_toply[i]+DIMENSION*el_ndof[2];// volume dof index
          KeM(indx,indx)  =det*(1-_bc_el[indx]);
          if(bc_s >3) {
            FeM(indx)  =  det*(1-_bc_el[indx])*_data_eq[_pres_order].ub[_pres_order*sur_toply[i]]*(1-_bc_el[indx]);
          }
        }
...
//                  _bc_el[sur_toply[lbnode]+ivar*NDOF_FEM]= ((_bc_vol[sur_toply[lbnode]]%10) == 1 ) ? 1:0 ; // storage bc for vol part
          ...
          // non-diagonal part
          for(int   jvar=ivar+_dir+1; jvar< _nFSIdim+ivar+_dir; jvar++)    {
            int      jvar1=jvar%DIMENSION;
            double aij=Ipenalty*normal[jvar1]*normal[ivar+_dir]*(bc_tg+bc_normal);
            double valj=_data_eq[2].ub[(_FSI_idx+jvar1)*NDOF_FEM+sur_toply[lbnode]];
...
//             int bc_yplus=((bc_s&8)>>3);  // (0;1??) nonhomogeneous
        ...
        if(ivar+_dir == dir_maxnormal)  {  // normal ---------------------------------------------------------------------------------------------------
          ...
          for(int   jvar=ivar+_dir+1; jvar< _nFSIdim+_dir+ivar; jvar++)    {  // non-diagonal part  ->  u_n=n(u*nx+v*ny+w*nz)
            int      jvar1=jvar%DIMENSION;
...
//             if( (_bc_vol[sur_toply[lbnode]]%10 != 0) || (_bc_el[sur_toply[lbnode]+ivar*NDOF_FEM]==1 && _bc_vol[sur_toply[lbnode]]%10 == 0)) {
          ...
          KeM(indx_row,indx_row) =  Ipenalty*(1-(1-bc_tg)*(1-bc_normal))-aii;

//           KeM(indx_row,indx_row) += -aii;
          FeM(indx_row) =  bc_rhs*bc_tg*Ipenalty*u_old[indx_sol]
                           -bc_tg*bc_rhs*aii*u_old[indx_sol];   // Dirichlet tang  homogeneous only
          // non-diagonal part
          for(int   jvar=ivar+_dir+1; jvar< _nFSIdim+ivar+_dir; jvar++)    {
