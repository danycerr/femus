...
// constant
double  _NSdensity2(double T,double T_r) {return 1.;} // water (t K) T_ref
double  _NSkviscosity2(double T,double T_r) {return 1.;} // lead T K
// water
...
void MGSolFSI::get_el_field_data(
  ...
  const int pres_order=(_nvars[0]>0)? 0:1;
  int el_ndofp=el_ndof[pres_order];
  ...
  _data_eq[2].mg_eqs[_data_eq[2].tab_eqs[FS_F]]->get_el_nonl_sol(0,_nFSIdim,el_ndof[2],el_conn, offset,0,u_nl);
  ...
  _data_eq[2].mg_eqs[_data_eq[2].tab_eqs[FS_F]]->get_el_sol(0,_nFSIdim,el_ndof[2],el_conn, offset,0,u_old);
  _data_eq[2].mg_eqs[_data_eq[2].tab_eqs[FS_F]]->get_el_oldsol(0,_nFSIdim,el_ndof[2],el_conn, offset,0,u_oold);
#endif

#if FSI_EQUATIONS==1 //   coupled ----------------------------------------------------------------------------------------
  
if (_pres_order==1)
  _data_eq[2].mg_eqs[_data_eq[2].tab_eqs[FS_F]]->get_el_sol(_nFSIdim,1,el_ndof[_pres_order],el_conn, offset,0,_data_eq[_pres_order].ub);    // pressure
  else 
  _data_eq[2].mg_eqs[_data_eq[2].tab_eqs[FS_F]]->get_el_sol_piece(_nFSIdim,1,el_ndof[_pres_order],iel, offset,0,_data_eq[_pres_order].ub);    // pressure
  
  _data_eq[2].mg_eqs[_data_eq[2].tab_eqs[FS_F]]->get_el_sol(0,_nFSIdim,el_ndof[2],el_conn, offset,0,u_old);     // old vel
  _data_eq[2].mg_eqs[_data_eq[2].tab_eqs[FS_F]]->get_el_nonl_sol(0,_nFSIdim,el_ndof[2],el_conn, offset,0,u_nl);   //  non linear vel
#endif

#if FSI_EQUATIONS==2 // pressure as external field (splitting) -----------------------------------------------------------
  for(int kdim=0; kdim< _nFSIdim; kdim++) {
    _data_eq[2].mg_eqs[_data_eq[2].tab_eqs[FS_F+kdim]]->get_el_nonl_sol(0,1,el_ndof[2],el_conn,offset,kdim,u_nl);
    _data_eq[2].mg_eqs[_data_eq[2].tab_eqs[FS_F+kdim]]->get_el_sol(0,1,el_ndof[2],el_conn, offset,kdim,u_old);
    _data_eq[2].mg_eqs[_data_eq[2].tab_eqs[FS_F+kdim]]->get_el_oldsol(0,1,el_ndof[2],el_conn, offset,kdim,u_oold);
  }
...
// ==============================================================================================
void MGSolFSI::matrixrhsvol_liq(DenseMatrixM &KeM, DenseVectorM &FeM,
...
// ==================================  Volume ===============================================
  ...
  int el_ndofp=el_ndof[_pres_order];
//     euler_impl=0.5;
  double rho=1.;
...
//        rho = 100*_vof_phase*1+(1-_vof_phase)*1.;
//       double mu_tp= 1000*_vof_phase+(1-_vof_phase);
  double sumk=0;
  for(int qp=0; qp<  el_ngauss; qp++) {
    ...
    for(int ik=0;ik<NDOF_FEM*DIMENSION;ik++) ff[ik]=0.;
    ...
    interp_el_gdx(_data_eq[2].ub,_FSI_idx, _nFSIdim,_dphi_g[2],el_ndof2,vel_gdx);    // derivatives  vel_gdx[DIM][DIM]
    interp_el_gddx(_data_eq[2].ub,_FSI_idx, _nFSIdim,_ddphi_g[2],el_ndof2,vel_gddx);

// #if (FSI_EQUATIONS%2==0)
//
// #endif
    // linear fields (pressure) -> projection and segregated
    interp_el_sol(_data_eq[_pres_order].ub,0,1,_phi_g[_pres_order],el_ndofp,_ub_g[_pres_order]);
#ifdef AXISYM
    ...
    for(int idim=0; idim<  _nFSIdim; idim++) {
      ...
      for(int k=0; k< NDOF_FEM; k++) {
        ...
        vel_g[idim] += _data_eq[2].ub[k+(idim+_FSI_idx)*NDOF_FEM]*_phi_g[2][k];
      }
    ...
    if(_T_idx>=0) {
      double Temp_g=_ub_g[2][_T_idx];   rho *= _NSdensity2(Temp_g,0);  mu *=  _NSkviscosity2(Temp_g,0);
    }
//     double IRe_eff=_IRe*mu;
    _mu_turb=0.;// eff visc turb at g pt
    // -------------------- Turbulence [K_F] -> (quad,_indx_eqs[K_F]) -------------------------------------
    if(_K_idx>=0) {// turbulence _mu_turb evaluation
      _kappa_g[0]= _ub_g[2][_K_idx];  _kappa_g[1]= _ub_g[2][_K_idx+1];
      _mu_turb=eval_var1(val_tbg);
    }
//   _mu_turb=fabs(exp(_kappa_g[0])/_kappa_g[1])/_IRe;
//  _mu_turb=0.002;
    double IRe_eff  =_IRe*mu_tp* (1.+_mu_turb)+  _IRe*  FSI_parameter.FSI_LES*0.01*h_eff*h_eff*sqrt(2*_sP)  ;   //  visc_eff  at g pt
...
//
      ...
      FeM(indx)  +=  dtxJxW_g*0.072*(ff[j+ivar*9]*phij_g)* (phii_g+Phi_supg);
          ...
          KeM(indx,j+ivar*el_ndof2) +=dtxJxW_g*rho* (
                                        (euler_impl*Adv_g+   phij_g/_dt)* (phii_g+Phi_supg)             // time + advection
        ...
        for(int  ikl=0; ikl<2; ikl++) {    // ikl=0 discontinuous ikl=1 continuous pressure
          for(int  ivarl=0; ivarl<_nvars[ikl]; ivarl++) {
            for(int  j=0; j<el_ndof[ikl]; j++) {
              ...
              KeM(indx,j+ _nFSIdim*el_ndof2) +=dtxJxW_g* (    // MPascal
...
//           if(_pres_order==0 ){ //internal boundary condition for piece pressure
//             std::cout<<"------- Neuman bc pres "<< _data_eq[_pres_order].ub[_pres_order*i] 
//             <<std::endl;
//           KeM(indx,indx)  =1./JxW_g2*(1-_bc_el[indx]); 
//           FeM(indx)  =  1./JxW_g2*_data_eq[_pres_order].ub[_pres_order*i]*(1-_bc_el[indx]);//(1-_bc_el[indx]); //    _u_old[DIMENSION]*KOMP_FSI;
//           }
         double dtxJxWp_g=JxW_g2*_bc_el[indx];//  KeM(indx,indx)  =JxW_g2*(1-_bc_el[indx]); // pressure bc
