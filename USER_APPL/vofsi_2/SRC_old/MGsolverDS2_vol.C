...
void MGSolDS::matrixrhsvol_sol_ds(
  ...
  for (int qp=0; qp< el_ngauss; qp++) {
    ...
    double  det1      = _fe[1]->Jac(qp,_xx_qnds,_InvJac1);     // Jacobian
    _fe[1]->get_phi_gl_g(_nDSdim,qp,_phi_g[1]);               // shape funct
    _fe[1]->get_dphi_gl_g(_nDSdim,qp,_InvJac1,_dphi_g[1]); // global coord deriv
    ...
    interp_el_sol(_data_eq[2].ub, _DS_idx, DIMENSION, _phi_g[2],el_ndof[2],_ub_g[2]);
    interp_el_sol(_data_eq[2].ub,_FSI_idx, DIMENSION, _phi_g[2],el_ndof[2],_ub_g[2]);
    // linear fields -> pressure
    // ------------------------------------------------------------------------------------------------------------
#if (FSI_EQUATIONS%2==0)  // projection and segregated
//         interp_el_sol(_data_eq[1].ub,0,1,_phi_g[1],el_ndofp,_ub_g[1]);
// #else                   // coupled
...
//       double dtxJxW_g=JxW_g2*(_bc_vol[i]%2)*_dt;
      double dtxJxW_g=JxW_g2*(_bc_el[i+NDOF_FEM*_dir])*_dt;
      const double   ds_old=_ub_g[2][_DS_idx+_dir];    const double   v_old=_ub_g[2][_FSI_idx+_dir];
      //         p_old=_ub_g[1][0];
      // rhs  Assemblying ---------------------------
//       FeM(i)+=dtxJxW_g*(u_old/_dt+v_old)*phii_g;
       FeM(i)+=dtxJxW_g*(v_old+ ds_old/_dt)*phii_g;
      // Matrix Assemblying ---------------------------
      KeM(i,i) += JxW_g2*(1-(_bc_el[i+NDOF_FEM*_dir]))*_dt;

      for (int j=0; j<el_ndof[2]; j++) {
        if (j<el_ndof[1])      for (int idim=0; idim< _nDSdim; idim++) {
            _dphijdx_g1[idim]=_dphi_g[1][j+idim*el_ndof[1]];
          }
// #ifdef ALE_MOVING
...
//          double phij_g= _phi_g[2][j];
        ...
        for (int kdim=0; kdim< _nDSdim; kdim++) {
          _dphijdx_g2[kdim]=_dphi_g[2][j+kdim*el_ndof[2]];
//           Lap +=_dphijdx_g2[kdim]*_dphiidx_g2[kdim]; // Laplacian
        }
        // energy-equation
        KeM(i,j) +=dtxJxW_g*(
                     phij_g*phii_g/_dt           //                          Lap
                   ) ;
//                        if(j<el_ndofp && time>2*_dt)
//             FeM(i)+=1.*dtxJxW_g*(dphijdx_g[1][_dir]*_data_eq[1].ub[j+NDOF_FEM])*phii_g*_dt;
...
void MGSolDS::matrixrhsvol_liq_ds(
  ...
  int flag_group[]
   ...
   double u_old; double v_old;
  const  int  el_ngauss = _fe[2]->_NoGauss1[ _nDSdim-1];                //elem gauss points
   if(3==3) for (int qp=0; qp< el_ngauss; qp++) {
...
//        if(interface==1) std::cout<< _bc_vol[i]<<" \n";
          double dtxJxW_g=JxW_g2*_bc_el[i]*fl_int*_dt;
          u_old=_data_eq[2].ub[(_DS_idx+_dir)*NDOF_FEM+i];
