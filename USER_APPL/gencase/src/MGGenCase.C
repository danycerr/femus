...
void MGGenCase::GenCase() {
  ...
  int dim=DIMENSION; std::cout << " Dimension= "<<  dim << "\n";
#ifdef PRINT_TIME    // ------------------------------------------------
  std::cout<< " \n ================================================ \n";
  ...
  const int  libmesh_gen = _mgutils.get_par("libmesh_gen");  // gen param flag
  switch(libmesh_gen) {  // mesh generator cases 0) File 1) Internal

  case 0: { // mesh from file at level 0 -----------------------
    ...
    if(_mgutils.get_par("second_order")==1) {
  ...
  case 1: { // Internal mesh generator at level 0 ---------------
    std::cout << " Internal mesh generator at level 0 \n";
    libMesh::ElemType libmname;                      // libmesh name elem type
    int  nintervx =0; int  nintervy =0; int  nintervz =0; // rectangular dimension

    switch(dim) {  // dimension cases 1,2,3 dim

    case 3:  { // dimension 3 --------------------------------------
      nintervx = _mgutils.get_par("nintervx");
      nintervy = _mgutils.get_par("nintervy");
      nintervz = _mgutils.get_par("nintervz");
    ...
    case 2: {  // dimension 2 --------------------------------------
      nintervx = _mgutils.get_par("nintervx"); // number of x-intervals
      nintervy = _mgutils.get_par("nintervy"); // number of y-intervals
  ...
  // ********************************************************
  const int  NoLevels = _mgutils.get_par("nolevels"); // param

  // type mesh map -----------------------------------
  int *ttype_FEM;     ttype_FEM=new int[2];
  ttype_FEM[0]=_geomel.n_q[0];  ttype_FEM[1]=_geomel.n_q[1];

  // mesh_map_in ----------------------------------
  int  *mesh_map_in;  mesh_map_in=new int [2*NoLevels];
  ...
  /// clean and stop
  // ********************************************************
  delete []ttype_FEM;  delete []mesh_map_in;
  delete bd_msht;  delete msht;
  delete bd_msh0; delete msh0;
...
//   const int  n_nodes_lev0=msh0.n_nodes();
  ...
  double *nod_val;  nod_val=new double[n_nodes*3];
  int *nod_flag;  nod_flag=new int[n_nodes];
  int *mat_flag;  mat_flag=new int[n_elements];
  int *map;  map=new int[n_elements*NDOF_FEM];
  int **elem_sto;  elem_sto=new int *[n_elements];
  int **elem_conn;  elem_conn=new int *[n_elements];
  int **bd_elem_sto;  bd_elem_sto=new int *[n_elements_b];
  int **nod_sto;  nod_sto=new int *[n_nodes];
  // order vectors -------------------
  ...
  int maptmp[NDOF_FEM];
  // zeroes ------------------
  for(int i=0; i< n_nodes; i++)   nod_flag[i]=0;   // bc condition
  for(int i=0; i<n_nodes*3; i++) nod_val[i]=0.;  // nodes
  for(int i=0; i<n_elements*NDOF_FEM; i++) map[i]=-1;  // nodes
  ...
  for(int ilev=0; ilev<_n_levels*2; ilev++) n_elements_lev[ilev]=0;  // nodes
  for(int ilev=0; ilev<_n_levels*2+1; ilev++) n_nodes_lev[ilev]=0;  // nodes
  for(int is=0; is<=_n_subdomains*_n_levels; is++) n_nodes_slp[is]=0;
  for(int i=0; i<n_elements; i++) {  // elements
  ...
  for(int i=0; i<n_elements_b; i++) {  // bd elements
...
//     int a= NDOF_FEM;
    ...
    for(int  s=0; s<(int) elem->n_sides(); s++) {
      ...
      if(elem->neighbor(s) == NULL)    {
        ...
        count_eb++;
      } else elem_conn[count_e][s+1]=(elem->neighbor(s))->id();
    }
    count_e++;
  }
...
//   const Mesh::const_element_iterator end_t00= msh0->elements_end();
  ...
  for(; it_t00 != end_t00; ++it_t00) {  // mesh00 processors
  ...
  status=H5Dread(dtset,H5T_NATIVE_INT,H5S_ALL,
                 H5S_ALL,H5P_DEFAULT,bc_condition_lev0);            // read
...
// Reading mat_condition ----------------------------------------------------
  ...
  status=H5Dread(dtset,H5T_NATIVE_INT,H5S_ALL,
                 H5S_ALL,H5P_DEFAULT,mat_condition_lev0);      // read
  assert(status==0);   // check
...
//   int * group_id_names =new int[1]; 
  ...
  for(int ielem=0; ielem<  n_elements_lev0; ielem++) {
    ...
    if(order_quad !=0) {
      ...
      for(int i=0; i<NDOF_FEM; i++) {  // loop over all child nodes
        int min=100000000;   int count=0;
        for(int j=0; j<NDOF_P; j++) {  // loop over father nodes
          int bc_tmp=nod_flag[maptmp[j]];

          if(MGGeomEl::Prol[j+i*NDOF_P]!=0) { // nodes with proj
            if(min>bc_tmp) min=bc_tmp;  // min wins
            count++;
          }
        }
        if(count<5)   nod_flag[maptmp[i]]=min;  // store the value
      }
// +++++++++++++++++++++++++++++
...
//     // bc and material *****************************************
  ...
  //clean
  delete []bc_condition_lev0;  delete []mat_condition_lev0;
  ...
  for(int ilev=0; ilev<_n_levels-1; ilev++) {       // level loop
    for(int ielem=0; ielem<n_elements; ielem++) {
      if(elem_sto[ielem][NDOF_FEM+1]==ilev) {       // only el with same level
        for(int ich=0; ich<elem_sto[ielem][NDOF_FEM+4]; ich++) {  // child loop
          ...
          for(int i=0; i<NDOF_FEM; i++) {  // loop over all child nodes
            ...
            for(int j=0; j<NDOF_P; j++) {  // loop over father nodes
              if(MGGeomEl::Prol[j+i*NDOF_P]!=0) { // nodes with proj
                if(min>bc_tmp[j]) min=bc_tmp[j];  // min wins
              }
            }
  ...
  for(int kel=0; kel<n_elements; kel++) {
    ...
    n_elements_sl[elem_sto[kel][NDOF_FEM+1]+elem_sto[kel][NDOF_FEM+2]*_n_levels]++;
    //setup ordering vector
    v_el[kel].first= elem_sto[kel][NDOF_FEM+1]+_n_levels*
                     elem_sto[kel][NDOF_FEM+2];
    v_el[kel].second=kel;
  }

  // std reordering on pairsNDOF_FEM
...
// ================================================
  ...
  for(int i=0; i<n_elements; i++) {
    for(int k=0; k<n_nodes_el; k++)   {
      ...
      if(elem_sto[i][NDOF_FEM+2] > nod_sto[knode][1])  nod_sto[knode][1]= elem_sto[i][NDOF_FEM+2];  // subdomain
      if(elem_sto[i][NDOF_FEM+1] < nod_sto[knode][2])  nod_sto[knode][2]= elem_sto[i][NDOF_FEM+1];  // level
      if(k <  NDOF_P &&  elem_sto[i][NDOF_FEM+1] < nod_sto[knode][3])  nod_sto[knode][3]= elem_sto[i][NDOF_FEM+1];
      nod_sto[knode][4]= 1;
    }
  }
...
// max number of element on each node
  ...
  int maxelxnode=0;
  for(int inode=0; inode<n_nodes; inode++) if(elxnode[inode]>maxelxnode) maxelxnode=elxnode[inode];
...
// clean
  ...
  for(int  ilev= 0; ilev< _n_levels; ilev++) {
    ...
    for(int iproc=0; iproc < _n_subdomains; iproc++) {
      for(int  inode=off_nd[0][iproc*_n_levels]; inode<off_nd[0][iproc*_n_levels+ilev+1]; inode++)
...
//   print mesh
  print_mesh_h5(n_nodes_lev, map_mesh_in, n_nodes, nod_val,
                ...
                off_el, v_inv_nd,  v_inv_el, elem_sto,elem_conn, v_el,bd_elem_sto,
                v_elb,bd_off_el, g_indexL,v, nod_flag,mat_flag);
...
#ifdef HAVE_MED
  const int  ibcflag = _mgutils.get_par("ibc_gen"); /// parameter defined in parameters.in
  ...
  if(ibcflag != 0) {
    ...
    print_med(_n_levels-1,bd_msht, msht ,n_groups_names, group_id_names,v,v_elb,nod_flag, mat_flag,str3.c_str());
    print_MedToMg(_n_levels-1,bd_msht, msht ,n_groups_names, group_id_names,v,v_inv_nd,v_elb,nod_flag, mat_flag,str3.c_str());
  }
#endif
...
// return;
// #else
  const int  mgops_gen = _mgutils.get_par("mgops_gen");

  if(mgops_gen) {
    compute_and_print_MGOps(n_nodes_lev,maxelxnode,off_el,v_el, elem_sto,
                            v_inv_nd,v_inv_el,g_indexL,off_nd) ;
  }
  // clean
  delete []v_inv_elb;
  delete []v_inv_el;
...
//  const int NDOF_FEM = Eltype.n_q[0]; const int max_elnd = Eltype.n_l[0];
  ...
  for(int i=0; i<n_elements; i++) {
    ...
    for(int k=1; k<1+NDOF_P; k++)  std::cerr<< elem_sto[i][k] << " ";  // nodes
    std::cerr<< ", ";
...
//   out << "[ <!ENTITY HeavyData \"mesh.h5\"> ] ";
  ...
  for(int ilev=0; ilev< _n_levels; ilev++) {
    ...
    out << "<DataStructure DataType=\"Int\" Dimensions=\"" <<  n_elements_lev[ilev]  << " " << NDOF_FEM <<"\" Format=\"HDF\">  \n";
    ...
    for(int idim=0; idim<3; idim++) {
      out << "<DataStructure DataType=\"Float\" Precision=\"8\" Dimensions=\""<< n_nodes_lev[_n_levels-1]<< " 1 \" Format=\"HDF\">  \n";
      out <<  basemesh << ".h5"//"mesh.h5"
    ...
    out <<"<DataItem DataType=\"Int\" Precision=\"8\" Dimensions=\""<< n_elements_lev[ilev]<< " 1 \" Format=\"HDF\">  \n";
...
// #endif
  ...
  for(int ilev=0; ilev< _n_levels; ilev++) {
    ...
    out << "<DataStructure DataType=\"Int\" Dimensions=\"" <<  n_elements_lev[_n_levels+ilev]   << " " << NDOF_FEMB <<"\" Format=\"HDF\">  \n";
    out <<  basemesh << ".h5"//"mesh.h5"
        << ":/ELEMS/FEM1/MSH"<< ilev <<  "\n";
    out << "</DataStructure> \n";
    out << "</Topology> \n";
    ...
    out <<"<DataStructure DataType=\"Int\" Precision=\"8\" Dimensions=\""<< n_nodes_lev[_n_levels-1]<< " 1 \" Format=\"HDF\">  \n";
...
void MGGenCase::print_mesh_h5(
  ...
  int bd_n_elements,int* off_el, int* v_inv_nd,int* v_inv_el,
  int** elem_sto, int** elem_conn,std::vector<std::pair<int,int> > v_el,
...
// _dim,_NoFamFEM,_NoLevels,Subdom ------------------
  // packaging data
  int *tdata;  tdata    = new int[4];
  tdata[0] = DIMENSION;  tdata[1] = 2;
...
// //  COORDINATES
  ...
  // _xyz ---------------------------------------------------------------------
  // packaging data
  double *xcoord;  xcoord = new double[n_nodes];
  ...
  for(int i=0; i< _n_subdomains; i++) {
    ...
    for(int j=0; j<_n_levels; j++) {
      std::cout << " Level " << j << " from " << off_nd[0][j+i*_n_levels]
  ...
  dimsf[0] =3;  dimsf[1] = 1;  int ndofm[3];
  ndofm[0]=NDOF_FEM;  ndofm[2]=NDOF_P;  ndofm[1]=0;
  ...
  // offset
  dimsf[0] = _n_subdomains*_n_levels+1;  dimsf[1] = 1;
  ...
  int *tempconn;  tempconn=new int[n_elements*NDOF_FEM]; // coonectivity all mesh
  ...
  int *tempconn_el;  tempconn_el=new int[n_elements*n_faces];
  for(int ielem=0; ielem<n_elements; ielem++) {
...
//       <<   v_inv_el[el_side] << std::endl;
      if(el_side !=-1) tempconn_el[iside+ielem*n_faces]=  v_inv_el[el_side];
      else tempconn_el[iside+ielem*n_faces]= -1;  // boundary element -1
    }
  }
  dimsf[0] = n_elements*n_faces;  dimsf[1] = 1; // global mesh hdf5 storage
  ...
  for(int ilev=0; ilev <_n_levels; ilev++) {
    ...
    for(int iproc=0; iproc <_n_subdomains; iproc++) {
      for(int iel=off_el[iproc*_n_levels+ilev];
        ...
        for(int inode=0; inode<NDOF_FEM; inode++) {
          tempconnf[ltot*NDOF_FEM+inode]=tempconn[iel*NDOF_FEM+
#if ELTYPE == 18
                                                  wedge_18[inode]
#else
                                                  inode
    ...
    dimsf[0] = n_elements_lev[ilev]*NDOF_FEM;    dimsf[1] = 1;
    name.str("");    name << "/ELEMS/FEM0/MSH"<<ilev ;
    status =_mgutils.print_Ihdf5(file,name.str(), dimsf,tempconnf);
    
    // material condition on lev (ilev)    ------------------------------------
    dimsf[0] = n_elements_lev[ilev];    dimsf[1] = 1;
    name.str("");    name << "/ELEMS/SUB/MAT"<<ilev ;
  ...
  // clean
  delete []tempconn;       delete []tempconn_el;
  delete []temp_material;
  H5Gclose(subgroup_id2);  H5Gclose(subgroup_id);
  // =+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
  ...
  dimsf[0] =3;  dimsf[1] = 1;
  ndofm[0]=NDOF_FEMB; ndofm[1]=0;  ndofm[2]=NDOF_PB;
  ...
  dimsf[0] = bd_n_elements*NDOF_FEMB;  dimsf[1] = 1;
  ...
  for(int ilev=0; ilev <_n_levels; ilev++) {
    ...
    for(int isubdom=0; isubdom<_n_subdomains; isubdom++) {
      for(int iel=bd_off_el[ilev+isubdom*_n_levels]; iel <bd_off_el[ilev+1+isubdom*_n_levels]; iel++) {
        for(int inode=0; inode<NDOF_FEMB; inode++) {
    ...
    // partial print -------------------------
    dimsf[0] =   ltot*NDOF_FEMB;    dimsf[1] = 1;
    name.str("");    name << "/ELEMS/FEM1/MSH"<<ilev ;
    status =_mgutils.print_Ihdf5(file,name.str(), dimsf,tempconnf);
    //clean
    delete []tempconnf;
  }
  ...
  for(int Level1 = 0; Level1 < _n_levels; Level1++) {
    ...
    for(int iql=0; iql<2; iql++) {
      ...
      for(int idom=0; idom<_n_subdomains; idom++) {
        offM[iql][idom+1]=offM[iql][idom]+(off_nd[iql][idom*_n_levels+Level1+1]-off_nd[iql][idom*_n_levels]);
      }
    }
    ...
    for(int idom=0; idom<=_n_subdomains; idom++) printf("  %d %d %d \n",idom,offM[0][idom],offM[1][idom]);
#endif
    ...
    nodes_top[2]=0;
    for(int kdom=0; kdom <_n_subdomains; kdom++) {  // piecewise
    ...
    for(int iql=0; iql<2; iql++) {
      for(int jql=0; jql<2; jql++) {
        ...
        int count_q=compute_mat_qq(Level1,level_row[iql],level_cln[jql], el_nodes[iql], el_nodes[jql],
                                   n_nodes,max_elnd,off_el,
                                   v_el,elem_sto,v_inv_nd,
                                   g_indexL,off_nd[iql],offM[qlt%2],aux,mem,Mat_q,len_q,len_qoff);
        print_op_h5(name.str(),nodes_top[iql],nodes_top[jql],count_q,Mat_q,NULL,
                    len_q,len_qoff,name_label);
      }
    }
    ...
    int jql=0;
    int qlt=20;
    int count_q=compute_mat_qk(Level1,level_row[iql],level_cln[jql], el_nodes[iql],el_nodes[2],
                               n_nodes,max_elnd,off_el,
                               v_el,elem_sto,v_inv_nd,
                               g_indexL,off_nd[iql],offM[iql],aux,mem,Mat_q,len_q,len_qoff);
    print_op_h5(name.str(),nodes_top[iql],nodes_top[2]/* *NDOF_K */,count_q,Mat_q,NULL,
                len_q,len_qoff,qlt);
    ...
    jql=0;
    qlt=02;
    count_q=compute_mat_kq(Level1,level_row[iql],level_cln[jql],el_nodes[2], el_nodes[jql],
                           n_nodes,max_elnd,off_el,
                           v_el,elem_sto,v_inv_nd,
                           g_indexL,off_nd[iql],offM[iql],aux,mem,Mat_q,len_q,len_qoff);
    print_op_h5(name.str(),nodes_top[2]/* *NDOF_K */,nodes_top[iql],count_q,Mat_q,NULL,
                len_q,len_qoff, qlt);
    ...
    jql=1;
    qlt=10;
    count_q=compute_mat_lk(Level1,level_row[iql],level_cln[jql], el_nodes[jql],el_nodes[2],
                           n_nodes,max_elnd,off_el,
                           v_el,elem_sto,v_inv_nd,
                           g_indexL,off_nd[1],offM[iql],aux,mem,Mat_q,len_q,len_qoff);
    print_op_h5(name.str(),nodes_top[iql],nodes_top[2]/* *NDOF_K */,count_q,Mat_q,NULL,
                len_q,len_qoff,qlt);
    ...
    jql=1;
    qlt=01;
    count_q=compute_mat_kl(Level1,level_row[iql],level_cln[jql],el_nodes[2], el_nodes[jql],
                           n_nodes,max_elnd,off_el,
                           v_el,elem_sto,v_inv_nd,
                           g_indexL,off_nd[iql],offM[iql],aux,mem,Mat_q,len_q,len_qoff);
    print_op_h5(name.str(),nodes_top[2]/* *NDOF_K */,nodes_top[iql],count_q,Mat_q,NULL,
    ...
    jql=0;
    qlt=0;
    count_q=compute_mat_kk(Level1,level_row[iql],level_cln[jql],el_nodes[2],el_nodes[2],
                           n_nodes,max_elnd,off_el,
                           v_el,elem_sto,v_inv_nd,
                           g_indexL,off_nd[iql],offM[iql],aux,mem,Mat_q,len_q,len_qoff);
    print_op_h5(name.str(),nodes_top[2]/* *NDOF_K */,nodes_top[2]/* *NDOF_K */,count_q,Mat_q,NULL,
                len_q,len_qoff, qlt);
...
void MGGenCase::compute_prol(
  ...
  for(int Level1 = 1; Level1 < _n_levels; Level1++) {
    ...
    if(Level1>1) level_cln[1]= Level1-2;  // linear
    level_row[2]=Level1;
    ...
    if(Level1>1) nodes_cln[1]=n_nodes_lev[Level1-2];  // linear
    ...
    for(int iql=0; iql<3; iql++) {  //[0]=quad  [1]=linear
      int count_q=0;
      if(iql<2) {
        ...
        count_q= compute_prol_qq(Level1,level_row[iql],level_cln[iql],el_nodes[iql],nodes_row[iql],
                                 off_el,v_el,elem_sto,v_inv_nd,g_indexL,off_nd[iql],
        ...
        count_q= compute_prol_kk(Level1,level_row[iql],level_cln[iql],el_nodes[iql],nodes_row[iql],
                                 off_el,v_el,elem_sto,v_inv_nd, v_inv_el,g_indexL,off_nd[iql],
                                 values_q,Prol_q,len_q,len_qoff);
      }

      print_op_h5(name.str(),nodes_row[iql],nodes_cln[iql],count_q,Prol_q,values_q,len_q,len_qoff,2-iql);
      // clean
      delete []Prol_q;
      delete[]values_q;
      delete []len_q;
...
void MGGenCase::compute_rest(
  ...
  for(int Level1 = 0; Level1 < _n_levels-1; Level1++) {
    ...
    if(Level1>0) nodes_row[1]=n_nodes_lev[Level1-1];  // linear
    ...
    for(int iql=0; iql<3; iql++) {  // [0]=quad [1]=linear [2]=piecewise
      int count_q=0;
      if(iql<2) {
        // set up
        ...
        count_q=compute_res_qq(Level1,level_row[iql],level_cln[iql],el_nodes[iql],
                               nodes_row[iql], nodes_cln[iql],max_elnd,
                               off_el,v_el,elem_sto, v_inv_nd,
                               g_indexL,off_nd[iql],
                               mem,values_q,Rest_q,  len_q, len_qoff);
      } else {
        // set up
        ...
        //  Restrictor constant --------------------------------------
        count_q=compute_res_kk(Level1,level_row[iql],level_cln[iql],el_nodes[iql],
                               nodes_row[iql], nodes_cln[iql],max_elnd,
                               off_el,v_el,elem_sto, v_inv_nd, v_inv_el,
                               g_indexL,off_nd[iql],
                               mem,values_q,Rest_q,  len_q, len_qoff);
      }
      // print ----------------------------------------------------
      print_op_h5(name.str(),nodes_row[iql],nodes_cln[iql],count_q,Rest_q,values_q,len_q,len_qoff,2-iql);
      // clean ----------------------------------------------------
      delete []Rest_q;
      delete []values_q;
...
int MGGenCase::compute_mat_kq(
  ...
  // zeroing matrix
  for(int im = 0; im < n_element_lev*ndof_row_ql; im++)   mem[im]=0;  // row index mem position
  ...
  // compressing
  for(int kdom=0; kdom <_n_subdomains; kdom++) {
    for(int iki=0; iki < off_el[kdom*_n_levels+Level1+1]-off_el[kdom*_n_levels+Level1]; iki++) {
      ...
      for(int kj=0; kj < max_elnd*ndof_clmn_ql ; kj++) {
        ...
        if(knod_q>=0 &&   aux[knod_q]==1) {
          ...
          if(knod_q< sum_lev*ndof_clmn_ql || knod_q >= (off_el[kdom*_n_levels+Level1+1]-
              off_el[kdom*_n_levels+Level1]+sum_lev)*ndof_clmn_ql)
            count_qoff++;
        }
      }
      len_q[ki+1]=count_q;
...
int MGGenCase::compute_mat_lk(
  ...
  for(int pr=0; pr<_n_subdomains; pr++) {
    for(int iel=off_el[pr*_n_levels+Level1]; iel<off_el[pr*_n_levels+Level1+1]; iel++) {
      int el_lib=v_el[iel].second;
      int sum_lev=0;
      for(int jpr=0; jpr<pr; jpr++) sum_lev +=off_el[jpr*_n_levels+Level1+1]-off_el[jpr*_n_levels+Level1];
      for(int kk=0; kk < ndof_row_ql; kk++) {
        int op= v_inv_nd[elem_sto[el_lib][1+kk]]; // libmesh number node
        int i_row= g_indexL[Level_row][op];       // my  number node
        for(int kj=0; kj < ndof_clmn_ql; kj++) {
    ...
    for(int iel=off_el[pr*_n_levels+Level1]; iel<off_el[pr*_n_levels+Level1+1]; iel++) {
      ...
      for(int kk=0; kk <  ndof_row_ql; kk++) {
        ...
        for(int kj=0; kj < ndof_row_ql; kj++) {
          ...
          if(ndof_row_ql== NDOF_P) values_q[g_indexL[Level_row][el_n]*NDOF_FEM+kj]=
              _geomel._embedding_matrix_l[ch_id][kk][kj];
          else    values_q[g_indexL[Level_row][el_n]*NDOF_FEM+kj]=
...
#else
            double val_qq=0;
            if(ndof_row_ql == NDOF_P)val_qq =_geomel._embedding_matrix_l[i_ch][kj][kk];
            else val_qq =_geomel._embedding_matrix_q[i_ch][kj][kk];
    ...
    for(int iel=off_el[pr*_n_levels+Level1]; iel<off_el[pr*_n_levels+Level1+1]; iel++) {
      ...
      for(int kk=0; kk < ndof_row_ql; kk++) {
        ...
        for(int i_ch=0; i_ch< n_childs; i_ch++) {
          ...
          int sum_levp1=0;
          for(int jpr=0; jpr<pr; jpr++) sum_levp1 +=off_el[jpr*_n_levels+Level1+2]-off_el[jpr*_n_levels+Level1+1];
...
void MGGenCase::print_med(
  ...
  int * conn; conn=new int [n_elements*NDOF_FEM];
  int * conn_bd; conn_bd=new int [bd_n_elements*NDOF_FEMB];
  int *elem_bd_id2; elem_bd_id2=new int [bd_n_elements];
  // element (nodes for element)
  int n_nodes_el; int bd_n_nodes_el;
  int count_eb=0;
      ...
      for(int  s=0; s<(int) elem->n_sides(); s++) {
        if(elem->neighbor(s) == NULL)    {
          ...
          for(int  ns=0; ns< bd_n_nodes_el; ns++) {
            ...
            conn_bd[count_eb*bd_n_nodes_el+ns]=side->node(nodesinvbd[ns]);
...
//     std::cout << igroup << "\n";
  ...
  // clean
  delete []coord; fama.clear(); a.clear();delete []conn;
...
void MGGenCase::print_MedToMg(
  ...
  name << input_dir << filename << "_gen.med" ;
  std::ostringstream name2;
  name2 << input_dir << filename << "_MedToMg.med" ;
  // coarse  mesh
  int n_nodes=msh0.n_nodes();          // from mesh
  int n_elements=msh0.n_elem();        // from mesh
  //  boundary coarse mesh
...
//   }
  ...
  ParaMEDMEM::MEDCouplingUMesh *mesh1=ParaMEDMEM::MEDCouplingUMesh::New("Mesh_1",_dim);
  mesh1->allocateCells(n_element_top);
  for(int  i = 0; i <  n_element_top; i++)  mesh1->insertNextCell(MED_EL_TYPE,n_nodes_el,conn+i*n_nodes_el);
  mesh1->finishInsertingCells();

  // MEDCouplingUMesh Mesh connectivity (boundary)
  ParaMEDMEM::MEDCouplingUMesh *mesh2=ParaMEDMEM::MEDCouplingUMesh::New("Mesh_1",DIMENSION-1);
  mesh2->allocateCells(bd_n_elements);
  for(int  i = 0; i < bd_n_elements; i++)  mesh2->insertNextCell(MED_EL_BDTYPE,bd_n_nodes_el,conn_bd+i*bd_n_nodes_el);
  mesh2->finishInsertingCells();
...
//     std::cout << igroup << "\n";
  ...
  std::cout << "\n\n ";
  mm->write(name2.str().c_str(),2);
  
  ParaMEDMEM::DataArrayDouble *Volnodi =  ParaMEDMEM::DataArrayDouble::New();
  int size = v.size();
  Volnodi->alloc(size,1);
  
  ParaMEDMEM::DataArrayDouble *Boundnodi =  ParaMEDMEM::DataArrayDouble::New();
  int sizeB = v_elb.size();
  Boundnodi ->alloc(sizeB,1);
  
  
//   mappanodi->fillWithValue(-1);
//   MEDLoader::WriteUMesh(name2.str().c_str(),mm->getMeshAtLevel(0),1);  
//   mappanodi->setInfoOnComponent(0,"new");
  
  // Here we store the Med to Mg map: j is the id of med node and v_inv_nd[j] is the mg id of the same node
  for(int j=0; j<size; j++) Volnodi->setIJ(j,0,v_inv_nd[j]);
  
//   for(int j=0; j<sizeB; j++) Boundnodi->setIJ(j,0,v_inv_nd[j]);
  
  
  ParaMEDMEM::MEDCouplingFieldDouble * field = ParaMEDMEM::MEDCouplingFieldDouble::New(ParaMEDMEM::ON_NODES);
  field->setMesh(mm->getMeshAtLevel(0));
  field->setArray(Volnodi);
  field->setName("VolNodes");
  MEDLoader::WriteFieldUsingAlreadyWrittenMesh(name2.str().c_str(),field);  
  ...
  std::cout<<"\033[1;31m Map Med to Mg printed in file "<<name2.str().c_str()<<"\033[0m\n";
  // clean
  delete []coord; fama.clear(); a.clear();delete []conn;
  coordarr->decrRef(); mesh1->decrRef(); mesh2->decrRef();
  mm->decrRef();
