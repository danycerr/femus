...
//       const double   ds_old=_ub_g[2][_FF_idx[SDSX_F]+_dir];    const double   v_old=_ub_g[2][_FF_idx[FS_F]+_dir];

      // rhs  Assemblying ---------------------------
        FeM(i)+=1000.*(
          _data_eq[2].ub[(_FF_idx[SDSX_F]+_dir)*NDOF_FEM+i]+ _data_eq[2].ub[(_FF_idx[FS_F]+_dir)*NDOF_FEM+i]*_dt
        );
       KeM(i,i) += 1000.;
    } // ----------------------------------------
...
void MGSolDS::matrixrhsvol_liq_ds(
  ...
  for(int qp=0; qp< el_ngauss; qp++) {
    ...
    for(int i=0; i<el_ndof[2]; i++)     {
      ...
      int fl_int= ((fabs(flag_group[i])>999 &&  fabs(_bc_el[i])==1)? 0:1);
