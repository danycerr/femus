...
//
        for(int j=0; j<el_ndof2; j++) {
          ...
          if (j<el_ndofp) {
            FeM(indx) += dtxJxW_g* (
                 -(p_proj[j]+dp_proj[j]/_dt)*_dphi_g[1][j+(ivar+_dir)*el_ndof[1]]*phii_g
                 -(p_proj[j]+dp_proj[j]/_dt)*_dphi_g[1][j+(ivar+_dir)*el_ndof[1]]*Phi_supg
...
//            KeM(indx,j+idimp1*el_ndof2) +=
           
           FeM(indx) -=    dtxJxW_g*rho*(IRe_eff*dphijdx_g2[ivar+_dir]*dphiidx_g2[idimp1]
...
//           KeM(indx,j+idimp2*el_ndof2) +=
                    FeM(indx) -=  dtxJxW_g*rho*(IRe_eff*dphijdx_g2[ivar+_dir]*dphiidx_g2[idimp2]
                         );
#endif    // ----------------------------------------------------------------------
...
//
  ...
  const double R_t = Re_t/CMU;                                             // turbulent Reynolds /_dtnumber
