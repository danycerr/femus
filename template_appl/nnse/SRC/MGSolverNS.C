...
// =====================================================================================================
  ...
  // NS parameters
  const int unsteady_flag=_NS_parameter.UNSTEADY;
  const int iaxisim=_NS_parameter.AXISYM;
  const double les=_NS_parameter.LES;
  // -------------------------- Geometry -------------------------------------------------------------
  ...
  int el_ndof[3];  el_ndof[0]=NDOF_K;  int elb_ndof[3];  elb_ndof[0]=0; // number of el dofs
...
//  for(int d=0; d< NDOF_P; d++) {_bc_el[_nNSdim*NDOF_FEM+d]=(_bc_vol[d]/10 ==1)?0:1; }
    ...
    if(_FF_idx[K_F]>=0) { _y_bcout=_NS_parameter.DIST_FIX;}//  _y_bcout=_mgmesh._dist[ iel+nel_b];
    ...
    for(int  iside=0; iside< el_sides; iside++) {
      if(el_neigh[iside] == -1) {
        ...
        // boundary condition
        set_bc_matrix(KeM,FeM,dir_maxnormal,sur_toply,el_ndof,elb_ndof, elb_ngauss,
                      normal,iaxisim);
//         if( x_m[0]>0.48) std::cout << x_m[0] << " " << x_m[1] << " " <<  x_m[2] << " "  << "bound Kem \n "<< KeM << "  \n" <<  iel << "\n bc rhs \n" <<  FeM;
      } // iside
    }  // -----------------------------  End Boundary -------------------------------------

    // ------------------------------ Volume --------------------------------------------
    matrixrhsvol(KeM,FeM,el_ndof,
                 u_old,u_oold, u_nl, p_proj,dp_proj,
                 unsteady_flag,iaxisim,les,mode);
//      if( x_m[0]>0.48)     std::cout << "vol KeM \n" << KeM << " ielem \n" <<  iel << " rhs \n" <<  FeM;
    // ---------------------- end volume (element) --------------------------------------

   double max_diag=0.; int count=0;
     for(int id=0; id< el_mat_nrows; id++) {
       if(_bc_el[id] !=0) if(max_diag<fabs(KeM(id,id))) { max_diag=fabs(KeM(id,id)); }
     }
     for(int id=0; id< el_mat_nrows; id++) if(_bc_el[id]==0) {
...
//    if( x_m[0]>0.48)     std::cout << " renorm KeM \n" << KeM << " ielem \n" <<  iel << " rhs \n" <<  FeM;
    // ----------------------------------------------------------------------------------
...
//
  ...
  int iter_nl=_NS_parameter.NL_ITER; if(time < _NS_parameter.NL_TIME0) { iter_nl=_NS_parameter.NL_ITER0; }
