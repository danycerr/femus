...
//     }
...
#if NS_EQUATIONS==0 // pressure as external field (projection)// --------------------------------------------------------------
  _data_eq[2].mg_eqs[_data_eq[2].tab_eqs[NS_F]]->get_el_nonl_sol(0,_nNSdim,el_ndof[2],el_conn, offset,0,u_nl);
  ...
  _data_eq[2].mg_eqs[_data_eq[2].tab_eqs[NS_F]]->get_el_sol(0,_nNSdim,el_ndof[2],el_conn, offset,0,u_old);
  _data_eq[2].mg_eqs[_data_eq[2].tab_eqs[NS_F]]->get_el_oldsol(0,_nNSdim,el_ndof[2],el_conn, offset,0,u_oold);
#endif
...
#if NS_EQUATIONS==2 // pressure as external field (splitting) -----------------------------------------------------------
  for(int kdim=0; kdim< _nNSdim; kdim++) {
    _data_eq[2].mg_eqs[_data_eq[2].tab_eqs[_FF_idx[NS_F]+kdim]]->get_el_nonl_sol(0,1,el_ndof[2],el_conn,offset,kdim,u_nl);
    _data_eq[2].mg_eqs[_data_eq[2].tab_eqs[_FF_idx[NS_F]+kdim]]->get_el_sol(0,1,el_ndof[2],el_conn, offset,kdim,u_old);
    _data_eq[2].mg_eqs[_data_eq[2].tab_eqs[_FF_idx[NS_F]+kdim]]->get_el_oldsol(0,1,el_ndof[2],el_conn, offset,kdim,u_oold);
  }
...
// #endif
    ...
    for(int i=0; i<el_ndof2; i++) {
      double hh=1.e-20; for(int idim=0; idim< _nNSdim; idim++) { hh += vel_g[idim]*_dphi_g[2][i+idim*el_ndof2]/mod2_vel; }
      h_eff += fabs(hh);
    }
    ...
    if(_FF_idx[T_F]>=0) {
      double Temp_g=_ub_g[2][_FF_idx[T_F]];   rho *= _NSdensity(Temp_g,0);  mu *=  _NSkviscosity(Temp_g,0);
    }
    ...
    double IRe_eff  =_IRe*mu*(1.+_mu_turb)+_IRe*les*h_eff*h_eff*sqrt(2*_sP)  ;   //  visc_eff  at g pt
    ...
    for(int i=0; i< el_ndof2; i++) {    // +++++++++++
      ...
      for(int  ivar=0; ivar< _nvars[2]; ivar++)    {
        ...
        for(int idim=0; idim< _nNSdim; idim++) {
          ...
          Phi_supg +=_NS_parameter.SUPG*_bc_el[indx]*f_upwind*u_nlg[idim]*dphiidx_g2[idim]; //f_upwind
...
//
        for(int j=0; j<el_ndof2; j++) {
          ...
          for(int kdim=0; kdim< _nNSdim; kdim++) {
            ...
            Lap_g    += (IRe_eff+_NS_parameter.UPWIND*f_upwind*u_nlg[kdim]*u_nlg[kdim]) *dphijdx_g2[kdim]*dphiidx_g2[kdim];
          ...
          const int idimp1= (ivar+1+_dir) % _nNSdim;
           KeM(indx,j+idimp1*el_ndof2) +=    dtxJxW_g*rho*(IRe_eff*dphijdx_g2[ivar+_dir]*dphiidx_g2[idimp1]
          ...
          KeM(indx,j+idimp2*el_ndof2) +=
            dtxJxW_g*rho*(IRe_eff*dphijdx_g2[ivar+_dir]*dphiidx_g2[idimp2]
...
//                                               -1.*psij_g*dphiidx_g2[ivar]
                                                _dphi_g[ikl][j+ivar*el_ndof[1]]*phii_g
...
//              if (mode == 1)   FeM(indx)  +=  JxW_g2*psii_g*_ub_g[1][0]*KOMP_FSI; //    _u_old[DIMENSION]*KOMP_FSI;
          double dtxJxWp_g=JxW_g2*_bc_el[indx];
