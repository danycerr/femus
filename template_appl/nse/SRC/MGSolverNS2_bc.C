    ...
    if(bc_dn==0) {  // dirichlet -------------------------------------------------------------------------------------------------------------------
      ...
      int bc_normal=(bc_s%2); int bc_tg=((bc_s&2)>>1); int bc_rhs=((bc_s&4)>>2);
        ...
        if(_dir == dir_maxnormal)  {  // normal ---------------------------------------------------------------------------------------------------
          ...
          FeM(indx_row) = bc_normal*bc_rhs*nii*_data_eq[2].ub[indx_sol+_FF_idx[NS_F]*el_ndof[2]]; // only normal non-homogeneous
          for(int   jvar=_dir+1; jvar< _nNSdim+_dir; jvar++)    {  // non-diagonal part  ->  u_n=n(u*nx+v*ny+w*nz)
          ...
          FeM(indx_row) =  Ipenalty*bc_rhs*bc_tg*(1-nii)*_data_eq[2].ub[indx_sol+_FF_idx[NS_F]*el_ndof[2]];  
          // non-diagonal part
          for(int   jvar=_dir+1; jvar< _nNSdim+_dir; jvar++)    {
            int      jvar1=jvar%_nNSdim;
