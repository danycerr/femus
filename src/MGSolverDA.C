...
// }
  ...
  _data_eq[1].tab_eqs[KTT_F]=n_index;                      //  table
...
//  end Reading  mat_id ******************************************************
    ...
    for(int iel=0; iel <off_el[off_proc+_NoLevels]-off_el[off_proc+_NoLevels-1]; iel++) {
      ...
      for(int i=0; i<NDOF_FEM; i++) {
        ...
        ic_read(face_id_node,mat_id_elem,xp,iel,u_value);//-----------------------==
        // -------------------------------------------------==
        // ===================================================
...
void MGSolDA::GenBc_loop(
  const int vb,         // 0-> volume   1-> boundary
...
//   const int pie_offset=(NSUBDOM)*(_mgmesh._NoElements[0][_NoLevels-1]); // fine level # of elements
  // field
  double *sol;
//   double *sol_pie;
//   hid_t  file_sol;
...
//       _mgutils.read_Dhdf5(file_sol,"/"+_var_names[ivar],sol);
      ...
      for(int iel=0; iel <_mgmesh._off_el[0][iproc*_NoLevels+_NoLevels]-
        ...
        for(int  i=0; i<el_nds; i++) {   // linear and quad
          int k=_mgmesh._el_map[0][elem_gidx+i];   // the global node
          const double value= (*x_old[_NoLevels-1])(_node_dof[_NoLevels-1][k+ivar*offset]);
...
//           _mgmesh._off_el[0][_iproc*_NoLevels+_NoLevels-1]; iel++) {
//         int  k= (iel+ndof_lev);
//       double value=  (*x_ooold[_NoLevels-1])(_node_dof[_NoLevels-1][k+ivar*offset]);
...
//   x[_NoLevels-1]->localize(*x_old[_NoLevels-1]);
  x[_NoLevels-1]->localize(*x_oold[_NoLevels-1]);
