    ...
    for(int  iel=0; iel < nel_e-nel_b; iel++) {
      ...
      for(int  iprocb=0; iprocb < _n_subdom; iprocb++) {
        ...
        for(int ielb=0; ielb < (nel_eb - nel_bb); ielb++) {
          ...
          // =================   user ========================== // distance using groups
          if(dq<dminq && (_bc_id[el_connb[NDOF_FEMB-1]] == 16 || _bc_id[el_connb[NDOF_FEMB-1]] == 15 ||
                          _bc_id[el_connb[NDOF_FEMB-1]] == 12 || _bc_id[el_connb[NDOF_FEMB-1]] == 13
                         )) { dminq=dq; }
          // =================== end user =======================
        }
      }
