...
//   _attrib.clear();                // Cell properties
  ...
  delete[] bc[0];  delete[] bc[1];                    // boundary condition flag
...
void MGSolBase::MGSolve(double Eps1,          // tolerance
...
// ===================================================================
  ...
  int NestedMG=1;
  if (NestedMG==0) {
    x[0]->zero();    MGStep(0,1.e-20,MaxIter,Gamma,Nc_pre,Nc_coarse,Nc_post);
    for (int Level = 1; Level < _NoLevels; Level++) {
      x[Level]->matrix_mult(*x[Level-1],*Prl[Level]); // projection
      rest= MGStep(Level,Eps1,MaxIter,Gamma,Nc_pre,Nc_coarse,Nc_post);              // MGStepsolution
    }
  } // ------------------------------------------------
...
//   MaxIter=10;//ATTENTION
  while (exit_mg==0 && cycle<MaxIter) { // multigrid step start -------
    ...
    rest= MGStep(_NoLevels-1,1.e-20,MaxIter,Gamma,Nc_pre,Nc_coarse,Nc_post);   // MGStep
...
// ====================================================================
  ...
  if (Level == 0) {   // coarse level ----------------------------------
    rest=_solver[Level]->solve(*A[Level],*x[Level],*b[Level],Eps1,200);
    ...
    rest=_solver[Level]->solve(*A[Level],*x[Level],*b[Level],Eps1, Nc_pre1);
    ...
    x[Level-1]->zero(); double coarser_rest;
    ...
    rest=_solver[Level]->solve(*A[Level],*x[Level],*b[Level],Eps1,Nc_post1);
