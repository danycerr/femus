...
void BoundInterp::FillParameters(
  ...
  InterpCoordNodes[27]=8;  InterpCoordNodes[9] =4;  InterpCoordNodes[3] =2;
...
// Number of nodes of source mesh per mesh element used for the calculation
  ...
  double Bbox[2*_SpaceDim];  SourceMesh->getBoundingBox(Bbox);
  double height = fabs(Bbox[3] - Bbox[2]);  double width =  fabs(Bbox[1] - Bbox[0]);

  double pointA[2];   pointA[0] = Bbox[0] - 1.e4*width;  pointA[1] = Bbox[2] - 1.e4*height;
  double pointB[2];  pointB[0] = Bbox[0] - 1.e4*width;  pointB[1] = Bbox[3] + 1.e4*height;

  DataArrayDouble *DDA = DataArrayDouble::New();  DDA -> alloc(_SrcCells,4); //  4 ????????????
...
//   std::vector<double> DDAM;   std::vector<double> DDAm; std::vector<double> DDBM;  std::vector<double> DDBm;
  ...
  const double coeffA = height/width;  const double coeffB = -height/width;
  ...
  double deltaAm = (coeffA - coeffAm)/3.;      double deltaAM = (coeffAM - coeffA)/3.;
  double deltaBm = fabs(coeffA - coeffAm)/3.;  double deltaBM = fabs(coeffAM - coeffA)/3.;
...
///////////////////////////////////////////////////////////////////////////////////////////////////
  ...
  for(int iCell=0; iCell < _SrcCells; iCell++) {
    ...
    DataArrayDouble *Dda = DataArrayDouble::New();    Dda -> alloc(_SrcCoordInterpNodes,1);
    DataArrayDouble *Ddb = DataArrayDouble::New();    Ddb -> alloc(_SrcCoordInterpNodes,1);
    ...
    for(int i_node=0; i_node < _SrcCoordInterpNodes; i_node++) {                // Loop over the element nodes
      ...
      for(int j =0; j<_SpaceDim; j++) { pos[j]= coord[j]; }
      ...
      if(iCell==4) { std::cout<<"inode "<< i_node <<" dda "<<dda[i_node] <<" ddb "<<ddb[i_node]<<std::endl; }
      coord.clear();
    }
    DDA->setIJ(iCell,0,Dda->getMinValueInArray()); DDA->setIJ(iCell,1,Dda->getMaxValueInArray());
    DDB->setIJ(iCell,0,Ddb->getMinValueInArray()); DDB->setIJ(iCell,1,Ddb->getMaxValueInArray());
    ...
    Dda->decrRef();    Ddb->decrRef();    TrgConn.clear();
  }


  // node loop -------------------------------------------------------------------------------------------
  int count=0;  int celleeee=0;
  std::map<int,int> MedLibmesh;  BuildCanonicalElementNodesMap(_SrcCellNodes, MedLibmesh);
  std::vector< double > coord;   std::vector<int> celle;   std::vector<int> valore;
...
// 	std::clock_t oldsearchbegin = std::clock();
    ...
    if(i_node == 4) for(int dim=0; dim<numero; dim++) { std::cout<< "dim " <<dim <<" cella " << celle[dim] <<std::endl; }
    if(numero>1.5) {
      celleeee += numero; count++;
      ...
      while(!found) {
        ...
        for(int SNode1 = 0; SNode1<_SrcCoordInterpNodes; SNode1 ++) {
          ...
          for(int direction = 0; direction<_SpaceDim; direction++) { _CoordMatrix.push_back(ElCoord1[direction]); }
          ElCoord1.clear();
        }
        XiEtaChiCalc(pos);
        if(fabs(_XiEtaChi[0])<1.000001&&fabs(_XiEtaChi[1])<1.000001) {
          ...
          _CoordMatrix.clear();  _XiEtaChi.clear();
        } else { _CoordMatrix.clear();   _XiEtaChi.clear();    }
        SourceConn.clear();
        i++;
      }
      ...
      for(int dim=0; dim<_MeshDim; dim++) { XiEta1->setIJ(i_node,dim,_XiEtaChi[dim]); }
      for(int SNode = 0; SNode<_SrcCellNodes; SNode++) {BoundingNodes1->setIJ(i_node, SNode, SourceConn[SNode]); }
      _CoordMatrix.clear();   _XiEtaChi.clear();    SourceConn.clear();
    } else {
      std::cout<<"nodo "<< i_node;
      std::cout<<"\033[1;31m Warning! zero cell found, tollerance = " << tollerance << "\033[0m\n";
      tollerance *=10.;   i_node += -1;
    }
    coord.clear();    celle.clear();    valore.clear();
  }
  ...
  short int subSec0[_SrcCells][9];  short int Scount0[9];
  short int subSec1[_SrcCells][9];  short int Scount1[9];
  short int subSec2[_SrcCells][9];  short int Scount2[9];
  short int subSec3[_SrcCells][9];  short int Scount3[9];

  for(int c=0; c<9; c++) {Scount0[c]=-1; Scount1[c]=-1; Scount2[c]=-1; Scount3[c]=-1;}

  short int* Scount[4];
  Scount[0] = & Scount0[0];  Scount[1] = & Scount1[0];  Scount[2] = & Scount2[0];  Scount[3] = & Scount3[0];

  short int* SubSec[4];
  SubSec[0] = & subSec0[0][0];  SubSec[1] = & subSec1[0][0];  SubSec[2] = & subSec2[0][0];  SubSec[3] = & subSec3[0][0];
  _Scount[0] = & Scount0[0];  _Scount[1] = & Scount1[0];  _Scount[2] = & Scount2[0];  _Scount[3] = & Scount3[0];
  _SubSec[0] = & subSec0[0][0];  _SubSec[1] = & subSec1[0][0];  _SubSec[2] = & subSec2[0][0];  _SubSec[3] = & subSec3[0][0];

#ifdef VERIFY_INTERP
  DataArrayDouble *OccupiedCells = DataArrayDouble::New();
  OccupiedCells -> alloc(_SrcCells,1);  OccupiedCells -> fillWithValue(0.);

  DataArrayDouble* FindNode = DataArrayDouble::New();
  FindNode->alloc(_TrgNodes,1);  FindNode->fillWithZero();
  DataArrayDouble* POS0 = DataArrayDouble::New();  POS0->alloc(_SrcCells,1);  POS0->fillWithValue(-1);
  DataArrayDouble* POS1 = DataArrayDouble::New();  POS1->alloc(_SrcCells,1);  POS1->fillWithValue(-1);
  DataArrayDouble* POS2 = DataArrayDouble::New();  POS2->alloc(_SrcCells,1);  POS2->fillWithValue(-1);
  DataArrayDouble* POS3 = DataArrayDouble::New();  POS3->alloc(_SrcCells,1);  POS3->fillWithValue(-1);

  _sec0 = DataArrayDouble::New();  _sec0->alloc(_SrcCells,9);  _sec0->fillWithValue(-1);
  _sec2 = DataArrayDouble::New();  _sec2->alloc(_SrcCells,9);  _sec2->fillWithValue(-1);
  _sec1 = DataArrayDouble::New();  _sec1->alloc(_SrcCells,9);  _sec1->fillWithValue(-1);
  _sec3 = DataArrayDouble::New();  _sec3->alloc(_SrcCells,9);  _sec3->fillWithValue(-1);

#endif

  std::vector< int > TrgConn;
  int Counts[4];  Counts[0] = -1;  Counts[1] = -1;  Counts[2] = -1;  Counts[3] = -1;
  double tollerance=1.e-4;  double toll=1.e-20;

  for(int iCell=0; iCell < _SrcCells; iCell++) {
    InMesh -> getNodeIdsOfCell(iCell,TrgConn);
    double ddamin =1.e+200; double ddamax = -1.e+200; double ddbmin=1.e+200; double ddbmax = -1.e+200;
    ...
    for(int i_node=0; i_node < _SrcCoordInterpNodes; i_node++) {                // Loop over the element nodes
      ...
      ddamin = (dda<ddamin? dda:ddamin); ddamax = (dda>ddamax? dda:ddamax);
      ddbmin = (ddb<ddbmin? ddb:ddbmin); ddbmax = (ddb>ddbmax? ddb:ddbmax);
      _coord.clear();
    }
    ...
    if((ddamax)>coeffA && (ddbmin)<coeffB) {
      ...
      _subsecAm = (int)((coeffAM-ddamin+toll)/deltaAM);      _subsecAM = (int)((coeffAM-ddamax+toll)/deltaAM);
      _subsecBm = (int)((coeffB -ddbmin+toll)/deltaBm);      _subsecBM = (int)((coeffB -ddbmax+toll)/deltaBm);
      _subsecBm = (_subsecBm==3?2:_subsecBm);               _subsecBM = (_subsecBM==3?2:_subsecBM);
      _subsecBm*=3;      _subsecBM*=3;
    ...
    if((ddamax)>coeffA && (ddbmax)>coeffB) {
      ...
      _subsecAm = (int)((coeffAM-ddamin+toll)/deltaAM);   _subsecAM = (int)((coeffAM-ddamax+toll)/deltaAM);
      _subsecBm = (int)((coeffBM-ddbmin+toll)/deltaBM);   _subsecBM = (int)((coeffBM-ddbmax+toll)/deltaBM) ;
      _subsecBm = (_subsecBm==3?2:_subsecBm);    _subsecBM = (_subsecBM==3?2:_subsecBM);
      _subsecBm*=3;      _subsecBM*=3;
    ...
    if((ddamin)<coeffA && (ddbmin)<coeffB) {
      ...
      _subsecAm = (int)((coeffA-ddamin+toll)/deltaAm);      _subsecAM = (int)((coeffA-ddamax+toll)/deltaAm);
      _subsecBm = (int)((coeffB-ddbmin+toll)/deltaBm);      _subsecBM = (int)((coeffB-ddbmax+toll)/deltaBm);
      _subsecBm = (_subsecBm==3?2:_subsecBm);              _subsecBM = (_subsecBM==3?2:_subsecBM);
      _subsecBm*=3;      _subsecBM*=3;
  ...
  std::vector<int>    valore;  std::vector<int>    SourceConn;
  int secA;  int secB;  int sector;  int Nsub;  int NsubsecB; int NsubsecA;
//   int count;
...
//
  ...
  for(int i_node=0; i_node < _TrgNodes; i_node++) {                // Loop over the element nodes
    ...
    _pos[0] = _coord[0];    _pos[1] = _coord[1];
    ...
    secA = (fabs(dda)>fabs(coeffA)? 1:0);    secB = (fabs(ddb)>fabs(coeffB)? 0:1);
    sector = secA + 2*secB; // 0 1 2 3

    if(sector==0) {  NsubsecA = (coeffA-dda)/deltaAm; NsubsecB = (coeffB-ddb)/deltaBm; NsubsecB*=3;}
    if(sector==1) {NsubsecA=(coeffAM-dda)/deltaAM; NsubsecB=(coeffB -ddb)/deltaBm; NsubsecB*=3;}
    if(sector==2) {NsubsecA = (coeffA-dda)/deltaAm; NsubsecB = (coeffBM-ddb)/deltaBM; NsubsecB*=3;}
    if(sector==3) {NsubsecA = (coeffAM-dda)/deltaAM; NsubsecB = (coeffBM-ddb)/deltaBM; NsubsecB*=3;}

    Nsub=NsubsecA+NsubsecB;
    bool found = false;
    int num=-1;
    while(!found) {
      num++;  posi = *(SubSec[sector]+num*9+Nsub);  // 9 ????????????????????????????
      if((DDA->getIJ(posi,0) -tollerance) < dda && (DDA->getIJ(posi,1) +tollerance) > dda &&
        ...
        for(int SNode1 = 0; SNode1<_SrcCoordInterpNodes; SNode1 ++) { InMesh -> getCoordinatesOfNode(SourceConn[SNode1],_CoordMatrix); }
...
// 	for(int SNode = 0; SNode<_SrcCellNodes; SNode++) BoundingNodes1->setIJ(i_node, SNode, SourceConn[SNode]);
    // clean
    _XiEtaChi.clear();    SourceConn.clear();    _coord.clear();    valore.clear();
...
//   MEDLoader::WriteFieldUsingAlreadyWrittenMesh("new_method.med",position);
  ...
  Sector->setMesh(InMesh);  Sector->setArray(POS0);  Sector->setName("Sector0");
  MEDLoader::WriteUMesh("MeshSectors.med",InMesh,true);
  MEDLoader::WriteFieldUsingAlreadyWrittenMesh("MeshSectors.med",Sector);

  Sector->setArray(POS1);  Sector->setName("Sector1");
  MEDLoader::WriteFieldUsingAlreadyWrittenMesh("MeshSectors.med",Sector);
  Sector->setArray(POS2);  Sector->setName("Sector2");
  MEDLoader::WriteFieldUsingAlreadyWrittenMesh("MeshSectors.med",Sector);
  Sector->setArray(POS3);  Sector->setName("Sector3");
  MEDLoader::WriteFieldUsingAlreadyWrittenMesh("MeshSectors.med",Sector);

  MEDLoader::WriteFieldUsingAlreadyWrittenMesh("MeshSectors.med",f);

  std::string subsectors[9];  subsectors[0]= "sub0";
  subsectors[1]= "sub1";  subsectors[2]= "sub2";  subsectors[3]= "sub3";  subsectors[4]= "sub4";
  subsectors[5]= "sub5";  subsectors[6]= "sub6";  subsectors[7]= "sub7"; subsectors[8]= "sub8";
  ...
  Sector->setArray(_sec0); Sector->setName("SubSector0");
  MEDLoader::WriteFieldUsingAlreadyWrittenMesh("MeshSectors.med",Sector);
  Sector->setArray(_sec1);  Sector->setName("SubSector1");
  MEDLoader::WriteFieldUsingAlreadyWrittenMesh("MeshSectors.med",Sector);
  Sector->setArray(_sec2);  Sector->setName("SubSector2");
  MEDLoader::WriteFieldUsingAlreadyWrittenMesh("MeshSectors.med",Sector);
  Sector->setArray(_sec3);  Sector->setName("SubSector3");
  MEDLoader::WriteFieldUsingAlreadyWrittenMesh("MeshSectors.med",Sector);

  //clean
  POS0->decrRef();  POS1->decrRef();  POS2->decrRef();  POS3->decrRef();  FindNode->decrRef();
  Node->decrRef();  Sector->decrRef();
...
//     int *nodalConnPerCell = new int[TrgCellNodes];
  double TCbbox[2*DIMENSION];   double minmax[2*DIMENSION];
  InitFe(); const int Quad4[8] = {-1, 1, 1,-1, -1,-1, 1, 1  }; // (xi,eta,zeta)
...
//       mesh->decrRef();
      ...
      for(int IntCellNum = 0; IntCellNum < SrcIntCells; IntCellNum++) {
        ...
        for(int i_node=0; i_node < NNodes; i_node++) {                 // Loop over the element nodes
          ...
          for(int dimension = 0; dimension < _SpaceDim; dimension ++) { XYZ->setIJ(i_node,dimension , ElCoord[dimension]); }
          ElCoord.clear();
        }
      ...
      for(int j = 0; j< TrgCellNodes; j++) {
        if(targetArray->getIJ(TrgConn[j],0)>0.5) {
          ...
          for(int direction = 0; direction<_SpaceDim; direction++) { NodeCoord[direction] = PointsCoords[j*_SpaceDim+direction]; }// real coordinates
            ...
            while(!CellFound && FirstTry<numberOfCell) { // ++++++++ +++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
              ...
              if(_MeshDim==2) { // ---------------------------------------------------------------------------------------------
                if(fabs(XiEtaBound[0]) <= 1.+1.e-5 &&  fabs(XiEtaBound[1]) <= 1.+1.e-5) {
                  ...
                  for(int node = 0; node<_SrcCellNodes; node++) { BoundingNodes1->setIJ(TrgConn[j], node, SrcNodesConn[node]); }
                } else {
                  ...
                  CellFound = false; FirstTry ++;
                  if(FirstTry!=numberOfCell) { XiEtaBound.clear(); } // clear the coords at the point
              ...
              if(_MeshDim==1) { // ---------------------------------------------------------------------------------------------
                if(fabs(XiEtaBound[0]) <= 1.+1.e-5) {
                  CellFound = true;
                  for(int node = 0; node<_SrcCellNodes; node++) { BoundingNodes1->setIJ(TrgConn[j], node, SrcNodesConn[node]); }
                } else { CellFound = false; FirstTry ++; XiEtaBound.clear();}
              } // ---------------------------------------------------------------------------------------------------------------
...
//             else
            ...
            for(int nMed = 0; nMed < SrcNodesConn.size(); nMed++)   if(_MeshDim==2) { Ordering[nMed] = nMed; }

            if(_MeshDim==1) {  Ordering[0] = 0;   Ordering[1] = 1; Ordering[2] = 2;}
            _CoordMatrix.clear();
            for(int SNode1 = 0; SNode1<_SrcCoordInterpNodes; SNode1 ++) {
              std::vector< double > ElCoord1;
...
//                   and the cell ids are elts[ eltsIndex[ i ]], elts[ eltsIndex[ i ] + 1 ].
    ...
    double Nodi[dimm];
    for(int dim=0; dim<dimm; dim++) { Nodi[dim] = CoordinatesOfNodes[dim]; }//
...
// controllare metodo di ricerca ed utilizzo degli array elts e eltsIndex -> guarda classe modificata da daniele
    __InMesh->getCellsContainingPoints(Nodi,_TrgNodes,1.e-3,elts,eltsIndex);
...
//             if(SNode <_SrcCoordInterpNodes)  __InMesh -> getCoordinatesOfNode(SourceConn[MedLibmesh[SNode]],_CoordMatrix); // only linear nodes
            ...
            if(SNode <_SrcCoordInterpNodes) { __InMesh -> getCoordinatesOfNode(SourceConn[SNode],_CoordMatrix); } // only linear nodes
          }
          XiEtaChiCalc(pos);

          if(fabs(_XiEtaChi[0]) > 1+1.e-3 || fabs(_XiEtaChi[1]) > 1+1.e-3 || fabs(_XiEtaChi[DIMENSION-1]) > 1+1.e-3) {
            found=false;
//             std::cout<<" Cell search failed after "<<iCount<<" attempts of "<<NumPossibleCells<<" \t node "<<TNode
//             <<" xi: "<<_XiEtaChi[0]<<" eta: "<<_XiEtaChi[1]  <<" chi: "<<_XiEtaChi[2]  <<std::endl;
            _XiEtaChi.clear(); _CoordMatrix.clear(); SourceConn.clear();
//             NodeCoord.clear();
          } else {
            found = true;
//             std::cout<<NumPossibleCells<<std::endl;
//             if(iCount>0) std::cout<<" Cell found after "<<iCount<<" attempts "
//                 <<" xi: "<<_XiEtaChi[0]  <<" eta: "<<_XiEtaChi[1]  <<" chi: "<<_XiEtaChi[2] <<std::endl;
          }
          iCount++;
        }// end while
...
//         std::cout<< " Cell not found for target point "<< TNode << " count "<<count << " x: "<<pos[0]<<" y: "<<pos[1]<<" z: "<<pos[2]<<std::endl;
      ...
      NodeCoord.clear();      SourceConn.clear();
    }
...
void BoundInterp::CheckBelonging(bool &found) {
  ...
  found = true;
  double S; double tildeS;
  double xm, ym, tildexm,tildeym, midX, midY ;
  for(int dim=0; dim<_SrcCoordInterpNodes; dim++) {
    int A = dim;
    int B = (4-(dim+1))%4;

    xm = 0.5*(xmedium - _pos[0]); xm = (fabs(xm<1.e-9)?1.e-9:xm);
    ym = 0.5*(ymedium - _pos[1]); ym = (fabs(ym<1.e-9)?1.e-9:ym);
    tildexm = 0.5*(_CoordMatrix[B*_SpaceDim] - _CoordMatrix[A*_SpaceDim]); tildexm = (fabs(tildexm<1.e-9)?1.e-9:tildexm);
    tildeym = 0.5*(_CoordMatrix[B*_SpaceDim+1] - _CoordMatrix[A*_SpaceDim+1]); tildeym = (fabs(tildeym<1.e-9)?1.e-9:tildeym);
    ...
    if(fabs(fabs(S)-1.)<toll || fabs(fabs(tildeS)-1.)<toll) {found=true; break;}    //coincident nodes
    if((fabs(S)+toll)<1.&&(fabs(tildeS)+toll)<1.) {found = false; break;}           //cross
  }
  return;
}
...
#ifdef VERIFY_INTERP
      if(sector==0) { _sec0->setIJ(iCell,_subsecAm+_subsecBm,1); }
      else if(sector==1) { _sec1->setIJ(iCell,_subsecAm+_subsecBm,1); }
      else if(sector==2) { _sec2->setIJ(iCell,_subsecAm+_subsecBm,1); }
      else if(sector==3) { _sec3->setIJ(iCell,_subsecAm+_subsecBm,1); }
#endif
...
void BoundInterp::GetXiEta(int node, std::vector<double> &XiEtabound) {
  for(int dim=0; dim<_MeshDim; dim++) { XiEtabound.push_back(XiEta->getIJ(node,dim)); }
  return;
}
// ===================================================================================
void BoundInterp::GetXiEtaChi(int node, std::vector<double> &XiEtaChi) {
  for(int dim=0; dim<_MeshDim; dim++) { XiEtaChi.push_back(XiEta->getIJ(node,dim)); }
  return;
}
...
void BoundInterp::XiEtaCalc_2D(
  ...
  // geometry
  double  xm[DIMENSION];              double normal[DIMENSION];
  double Co[DIMENSION*DIMENSION];  double Coord[NDOF_FEMB*DIMENSION];
  ...
  double XYZm[DIMENSION];  double XYZs[DIMENSION];
  double xieta[_MeshDim]; // reference point coordinates
  for(int kdim=0; kdim<_SpaceDim; kdim++) {
    XYZm[kdim]=0.;
    Co[kdim*_SpaceDim   ] =  1.e-20; Co[kdim*_SpaceDim +1] =  1.e-20; Co[kdim*_SpaceDim +2] =  1.e-20;
  ...
  if(fabs(Co[1+3*id1])+ fabs(Co[0+3*id1]) <1.e-10)   {
    int itmp= id1; id1=id2; id2=itmp;   std::cout << " index inversion "<< std::endl;
  }
  ...
  if(fabs(alpha_0) >1.e-10) { //  alpha_0 != 0 two roots 1.1e-10 ==========================================
    ...
    if(fabs(Co[1+3*id1]+Co[0+3*id1]*xieta[1])<1e-10) { xieta[0]=-(-XYZs[id2]+Co[2+3*id2]*xieta[1])/(Co[1+3*id2]+Co[0+3*id2]*xieta[1]); }
...
void BoundInterp::XiEtaCalc_1D(
  ...
  double xm = 0.5*(_CoordMatrix[0]+_CoordMatrix[2]);    double ym = 0.5*(_CoordMatrix[1]+_CoordMatrix[3]);
  double a  = 0.5*(-_CoordMatrix[0]+_CoordMatrix[2]);   double b  = 0.5*(-_CoordMatrix[1]+_CoordMatrix[3]);
...
void BoundInterp::XiEtaChiCalc(double NodePos[]) {
  double XiEtaChi[_MeshDim]; for(int dim=0; dim<_MeshDim; dim++) { XiEtaChi[dim] = 0.; }
    ...
    if(_MeshDim==3) {
      ...
      for(int row=0; row<_MeshDim; row++) {
        for(int col=0; col<_MeshDim; col++) {
          ...
          } else { InvHex[row*_MeshDim + col] = InvHex[col*_MeshDim + row]; }
        }
      }
    }
...
//   _CoordHex27
  double value = _XiEtaChiVert[node+dir*_VertCoordOff];
  for(int dim=0; dim<_MeshDim; dim++) {
    if(dim == dir) { value *=0.5; }
    else { value *= 0.5*(1.+_XiEtaChiVert[node + dim*_VertCoordOff]*XiEtaChi[dim]); }
  }
  return value;
}
...
double BoundInterp::SecondDer(double XiEtaChi[],int node, int row, int col) {
  ...
  double value;
  if(row==col) { value=0.; }   /// Second derivatives like d2N/dxidxi are equal to zero for linear test functions
  else {
...
double BoundInterp::LinPhi(int node, double XiEtaChi[]) {
  double N = 1.;
  for(int dir=0; dir<_MeshDim; dir++) { N *= 0.5*(1.+XiEtaChi[dir]*_XiEtaChiVert[node+dir*_VertCoordOff]); }
  return N;
}
...
double BoundInterp::CalcF(double NodePos[], double XiEtaChi[]) {
  ...
  for(int i = 0; i<_SpaceDim; i++) {
    double SUM = 0.;
    for(int j=0; j<_SrcCoordInterpNodes; j++) { SUM += _CoordMatrix[j*_SpaceDim + i]*LinPhi(j, XiEtaChi); }
...
void BoundInterp::FirstDerF(double NodePos[], double XiEtaChi[]) {
  ...
  for(int extdir=0; extdir<_SpaceDim; extdir++) {
    Error[extdir] = 0.;
    double xyzInterp = 0.;
    for(int nNode = 0; nNode < _SrcCoordInterpNodes; nNode++) { xyzInterp += _CoordMatrix[nNode*_SpaceDim + extdir]*LinPhi(nNode, XiEtaChi); }
  ...
  for(int comp = 0; comp<_SpaceDim; comp++) {
    ...
    for(int dir=0; dir<_SpaceDim; dir++) {
      double dxyzInterp = 0.;
      for(int nNode = 0; nNode < _SrcCoordInterpNodes; nNode++) { dxyzInterp += _CoordMatrix[nNode*_SpaceDim + dir]*FirstDer(XiEtaChi,nNode,comp); }
    ...
    for(int col=0; col<_MeshDim; col++) {
      ...
      if(row<=col) {
        for(int dir=0; dir<_MeshDim; dir++) {
          ...
          if(col==row) { DerValue += 2.*Der1*Der2; }
          else { DerValue += -2.*(Error[dir]*d2xyzInterp - Der1*Der2); }
        }
      } else {DerValue = _d2F[col*_MeshDim+row];}
...
BoundInterp::InterpolatedField(const ParaMEDMEM::MEDCouplingFieldDouble* SourceField, const int order) {
  ...
  case(Boundary):
    ...
    for(int iNode = 0; iNode<_TrgNodes; iNode ++) {
      ...
      std::vector<int> BoundingNodes; GetInterNodes(iNode,  BoundingNodes);
      std::vector<double> XiEtaBound; GetXiEta(iNode, XiEtaBound);
      
      
      
      
      double CanPos[_MeshDim];   for(int dim=0; dim<_MeshDim; dim++) { CanPos[dim]=XiEtaBound[dim]; }
  ...
  case(Volume):
    ...
    for(int iNode = 0; iNode<_TrgNodes; iNode ++) {
      ...
      if(BoundingNodes[0] == -1) {
        for(int icomp=0; icomp<NComp; icomp++) { targetArray->setIJ(iNode,icomp,100.); } //(node id, comp, value)
